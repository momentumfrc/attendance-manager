...
export class ShowStudentComponent implements OnInit, OnDestroy {
  ...
  protected sessionsSubject = new ReplaySubject<AttendanceSession[]|null>(1);
  ...
  protected attendanceStats = new ReplaySubject<AttendanceStats|null>(1)
  ...
  ngOnInit(): void {
    ...
    combineLatest({
      ...
      map(({sessions, meetings}) => sessions ? new AttendanceStats(sessions, meetings) : null)
